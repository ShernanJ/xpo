...
const DEFAULT_COOLDOWN_MS = SCRAPER_RATE_LIMIT_COOLDOWN_MS;
...
export function printUsage() {
  console.error(
    [
      "Usage:",
      "  node --experimental-strip-types --experimental-specifier-resolution=node scripts/scrape-user-tweets-http.ts --account <username> [options]",
      "",
      "Options:",
      ...
      "  --session <value>         Force a specific scraper account id or username.",
      "  --session-file <path>     Legacy no-op option kept for compatibility.",
      ...
      "",
      "Examples:",
      "  node --experimental-strip-types --experimental-specifier-resolution=node scripts/scrape-user-tweets-http.ts --account shernanjavier --import",
      "  node --experimental-strip-types --experimental-specifier-resolution=node scripts/scrape-user-tweets-http.ts --account shernanjavier --cookie 'auth_token=...; ct0=...' --import",
    ].join("\n"),
  );
}
...
function shouldCooldownSession(error) {
  return error instanceof RateLimitError || error instanceof SessionDeadError;
}
...
function shouldRetryWithAnotherSession(error) {
  ...
  return (
    error instanceof NoHealthySessionError ||
    error instanceof RateLimitError ||
    error instanceof SessionDeadError ||
...
async function fetchJsonWithRetry(url, options, retries = 1, failureContext = null) {
  let lastError = null;
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 401 || response.status === 403 || response.status === 429) {
          await handleScraperResponseFailure({
            accountId: failureContext?.accountId ?? null,
            status: response.status,
            bodyPreview: text.slice(0, 300),
            context: failureContext?.context ?? "X request",
          });
        }
...
async function resolveUserRestIdFromGraphqlUserByScreenName(params) {
  const { account, userAgent, headers, queryId: providedQueryId, accountId } = params;
...
async function resolveUserRestIdFromProfilePage(params) {
  const { account, userAgent, cookie, accountId } = params;
...
async function runScrapeAttempt(params) {
  ...
  const accountId = sessionHandle.accountId ?? null;
...
export async function runUserTweetsCapture(rawOptions) {
  ...
  const hasManualAuthOverride = Boolean(
    options.cookie || options.csrf || options.bearer || options.forceGuest,
  );
  ...
  try {
    ...
    for (;;) {
      const sessionHandle = await broker.acquire({
        forcedSessionId,
        requireAccount: !hasManualAuthOverride,
      });
      finalAccountId = sessionHandle.accountId ?? finalAccountId;

      if (
        !forcedSessionId &&
        sessionHandle.accountId &&
        attemptedAccountIds.has(sessionHandle.accountId)
      ) {
        throw new NoHealthySessionError(
          "All healthy scraper accounts were exhausted during this scrape.",
          Array.from(attemptedAccountIds),
        );
      }
      ...
      try {
        ...
        attemptedAccountIds.add(sessionHandle.accountId);
        rotatedAccountIds.push(sessionHandle.accountId);
        console.warn(
          `[session] Scraper account ${sessionHandle.label} failed; retrying the scrape from page 1 with another account.`,
        );
    ...
    const payload = attachScrapeMeta(finalPayload, {
      ...
      rotatedAccountIds,
      didRotateAccount: rotatedAccountIds.length > 0,
